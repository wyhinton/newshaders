...
void main()
{
    ...
    st = getRotation(st, vec2(center), -u_time);
    ...
    float timeFactor = 3.14 * 0.2 * cos(u_time * 0.5);
    ...
    st = getRotation(st, vec2(center), 3.14 * 0.1 * sin(u_time));
    ...
    float innerBox = getBox(st, -.04, -0.04, 0.08, 0.08);
    ...
    float sideMarks = step(18., mod(floor((st.y + 0.1 * sin(u_time)) / 0.002), 20.0));
    sideMarks *= (
    ...
    box *= (0.2 + 0.8 * pow(abs(sin(u_time * 4.)), 2.));
