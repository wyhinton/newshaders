...
float makeRings(vec2 p, float ringRadius){
    float st = sin(u_time);
    vec2 endP = abs(rsym(rotate(p, PI/6.), 6.)) - vec2(ringRadius, 0);
    float fOut = shape_circle(endP, .1);
    return fOut;
}
...
float SAMPLER(vec2 p) {
    ...
    float s1 = sub(add(cylinder_fill, cylinder_back), cylinder_cut);
    // float s1 = rsym()
    vec2 p12 = abs(rsym(rotate(p, PI/6.), 6.)) - vec2(.8, 0);
    // vec2 p12 = abs(rsym(rotate(p, PI/6.), 6.)) - vec2(.6, 0);
    // float cin  = add(shape_circle(p12, .075), shape_rect(p12, p12));
    float cin  = shape_rect(p12, p12+.18);
    s1 = add(cin, s1);
    float st = sin(u_time);
    float toAdd = makeRings(p, abs(1.*st));
    s1 = add(s1, toAdd);
