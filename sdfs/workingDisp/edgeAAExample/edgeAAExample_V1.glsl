...
{ 
    vec3 col = castRay(ro,rd,20.0, 1. - u_mouse.x);
...
void main()
{
    ...
    vec3 col = render( vec3(0.0, 0.9333, 1.0), rd );

	col = sqrt( col );

	
    fragColor=vec4( col, 1.0 );
    // fragColor=vec4( cw, 1.0 );
    // fragColor=vec4( cw, );
    // fragColor=vec4( rd, 1.0 );
    // fragColor=vec4( ro, 1.0 );
	// vec3 tColor = vec3(0.0, 0.9333, 1.0);
}